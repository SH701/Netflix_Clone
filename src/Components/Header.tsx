...
`
const navVar = {
  up:{backgroundColor:"rgba(0,0,0,0.8)"},
 ...
 function Header() {
    ...
    useMotionValueEvent(scrollY, "change", (latest) => {
      if(latest>80){
        navControls.start("up")
      }else{
        navControls.start("scroll")
      } 
    })
  ...
  const toggleLanguage = () => {
    const newLang = lang === "en-US" ? "ko-KR" : "en-US";
   ...
   return (
    ...
    >
      <Col>
         ...
         <Item>
            <Link to="/">
              <H1>{isKorean ? "홈" : "Home"}</H1>
            </Link>
            {pathname === "/" && <Circle layoutId="Circle" />}
          </Item>
          <Item>
            <Link to="/tv"><H1>{isKorean?"TV 프로그램":"TV Shows"}</H1></Link>
      ...
      <Col>
        <Search onSubmit={handleSubmit(onValid)}>
          ...
          {...register("keyword",{required:true,minLength:2})}
           ...
           initial={{ scaleX: 0 }}
           transition={{ ease: 'linear' }} 
           placeholder="Let’s find movie or show!!"/>
        </Search>
        <Btn onClick={toggleLanguage}>
      {lang === "en-US" ? "한국어" : "ENGLISH"}
      </Btn>
      </Col>
    </Nav>
  );
